    ...
    <div className="grid grid-cols-2 gap-3">
      {platforms && platforms.length > 0 ? (
        platforms.map((p, i) => {
          ...
          const isHE = name.includes("hackerearth") || (p.url || "").includes("hackerearth.com");
          const usernameGuess = (p.url || "").split("@")[1] || (p.handle || p.username || "");

          const inner = (
            <div className="flex items-center gap-2 bg-[var(--card)] rounded-lg p-3 shadow-sm hover:shadow-md transition">
              {/* Icon fallback */}
              <img
          ...
          return isHE && usernameGuess ? (
            <Link key={i} to={`/dashboard/hackerearth/${usernameGuess}`}>{inner}</Link>
          ) : (
            <a
              key={i}
              href={p.url || "#"}
